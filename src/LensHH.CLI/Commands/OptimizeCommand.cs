...
namespace LensHH.CLI.Commands
{
  ...
  elitehops (default 150) + elitefactor (default 10) gate the reseed-from-elite rescue: a chain is handed another chain's design only after elitehops hops with no best of its own AND its own best being worse than elitefactor x the best design found so far. Both must hold — the hop count alone fires on healthy chains and collapses the population onto one design. elitehops=0 disables it.
  [green]optimize preview [[engine=native|csharp]] [[analytic=true|false]] [[broyden=true|false]] [[gpuimage]][/]  Report which merit/Jacobian engine a run would ACTUALLY use with these settings, and the reason for any downgrade. Same query the GUI's Preview button makes.";
        ...
        public void Execute(Session session, string[] args)
        {
            ...
            // LENSHH_LM_DEBUG is set. Set for EVERY subcommand, not just 'run': the inner LM of a
            // basin/multistart hop is exactly where a stall needs to be observed.
        ...
        private void RunMultistart(Session session, string[] args)
        {
            ...
            for (int i = 1; i < args.Length; i++)
            {
                ...
                switch (key)
                {
                    ...
                    case "mincurvchange":
                        ...
                        break;
                    case "gpufill":
                        if (double.TryParse(val, NumberStyles.Float, CultureInfo.InvariantCulture, out double gpf))
                            settings.GpuPreScreenFill = gpf;
                        break;
                    case "gpuimage": gpuImage = true; break;
                }
            }
            ...
            if (settings.UseGpuPreScreen)
            {
                string gateTxt = settings.GpuPreScreenMinCurvatureChangePercent > 0
                    ...
                    : "off";
                AnsiConsole.MarkupLine($"  GPU pre-screen: ON  (difference gate {gateTxt}, population fill ×{settings.GpuPreScreenFill:G3})");
            }
        ...
        private void RunBasinHopping(Session session, string[] args)
        {
            ...
            for (int i = 1; i < args.Length; i++)
            {
                ...
                switch (key)
                {
                    ...
                    // Reseed-from-elite thresholds. BOTH must hold: elitehops alone is not a
                    // stall (a Metropolis walk goes tens of hops between records while working
                    // fine), and triggering on it alone collapses every chain onto the leader.
                    // elitefactor is the "genuinely out of contention" test that keeps the
                    // population diverse. elitehops=0 disables the trigger.
                    case "elitehops": if (int.TryParse(val, out int eh_settings)) settings.EliteRestartHops = eh_settings; break;
                    case "elitefactor": if (double.TryParse(val, NumberStyles.Float, CultureInfo.InvariantCulture, out double ef_settings)) settings.EliteRestartMeritFactor = ef_settings; break;
        ...
        private void RunGlobalBasinHopping(Session session, string[] args)
        {
            ...
            for (int i = 1; i < args.Length; i++)
            {
                ...
                switch (key)
                {
                    ...
                    // elitefactor is the "genuinely out of contention" test that keeps the
                    // population diverse. elitehops=0 disables the trigger.
                    case "elitehops": if (int.TryParse(val, out int eh_s)) s.EliteRestartHops = eh_s; break;
                    case "elitefactor": if (double.TryParse(val, NumberStyles.Float, CultureInfo.InvariantCulture, out double ef_s)) s.EliteRestartMeritFactor = ef_s; break;
