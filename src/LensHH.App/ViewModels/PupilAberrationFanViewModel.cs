...
public partial class PupilAberrationFanViewModel : ObservableObject
{
    ...
    public PupilAberrationFanViewModel(GuiSession session)
    {
        _session = session;
    }
    ...
    private static Bitmap RenderToBitmap(PupilAberrationResult[] results, string[] fieldLabels,
        string[] waveLabels, int numWavelengths)
    {
        ...
        // Minimum scale to avoid amplifying numerical noise
        ...
        for (int fi = 0; fi < results.Length; fi++)
        {
            ...
            string tanSvg = PupilAberrationFanRenderer.RenderFan(results[fi].TangentialFan,
                label + " \u2014 Tangential",
                maxAberration, numWavelengths, options);
            DrawSvgOnCanvas(canvas, tanSvg, 5, yOff);

            string sagSvg = PupilAberrationFanRenderer.RenderFan(results[fi].SagittalFan,
                label + " \u2014 Sagittal",
                maxAberration, numWavelengths, options);
    ...
    public async Task ExportText()
    {
        ...
        var file = await topLevel.StorageProvider.SaveFilePickerAsync(
            ...
            {
                Title = "Export Pupil Aberration Fan Data",
        ...
        for (int f = 0; f < _lastResults.Length; f++)
        {
            ...
            sb.AppendLine(PupilAberrationFanTextExport.Export(_lastResults[f],
